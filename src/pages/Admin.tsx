...
export default function Admin() {
  ...
  const openEditModal = (product: Product) => {
    setFormData({
      ...
      stock38: (product.stock_por_talla['38'] || 0).toString(),
      stock39: (product.stock_por_talla['39'] || 0).toString(),
      stock40: (product.stock_por_talla['40'] || 0).toString(),
      stock42: (product.stock_por_talla['42'] || 0).toString(),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      stock_por_talla: {
        '38': parseInt(formData.stock38) || 0,
        '39': parseInt(formData.stock39) || 0,
        '40': parseInt(formData.stock40) || 0,
        '42': parseInt(formData.stock42) || 0,
      },
  ...
  const getTotalStock = (product: Product) => {
    return Object.values(product.stock_por_talla || {}).reduce((acc, curr) => acc + (Number(curr) || 0), 0)
  }

  const totalStock = products.reduce((sum, p) => sum + getTotalStock(p), 0)
  const totalValue = products.reduce((sum, p) => sum + p.precio * getTotalStock(p), 0)
          ...
          <table className="w-full">
            <thead className="border-b border-border bg-muted/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold">Stock Total</th>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-background p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="border border-border rounded-lg p-4">
                <label className="text-sm font-medium mb-2 block">Stock por Talla</label>
                <div className="grid gap-2 grid-cols-4">
                  <div>
                    <label className="text-xs text-muted-foreground">38</label>
                    <input
                      type="number"
                      value={formData.stock38}
                      onChange={(e) => setFormData({ ...formData, stock38: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-2 text-sm focus:border-primary focus:outline-none"
                      min="0"
                      required
                    />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">39</label>
                    <input
                      type="number"
                      value={formData.stock39}
                      onChange={(e) => setFormData({ ...formData, stock39: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-2 text-sm focus:border-primary focus:outline-none"
                      min="0"
                      required
                    />
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground">40</label>
                    <input
                      type="number"
                      value={formData.stock40}
                      onChange={(e) => setFormData({ ...formData, stock40: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-2 text-sm focus:border-primary focus:outline-none"
                      min="0"
                      required
                    />
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground">42</label>
                    <input
                      type="number"
                      value={formData.stock42}
                      onChange={(e) => setFormData({ ...formData, stock42: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-2 text-sm focus:border-primary focus:outline-none"
                      min="0"
                      required
                    />
